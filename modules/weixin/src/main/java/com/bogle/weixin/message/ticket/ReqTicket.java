package com.bogle.weixin.message.ticket;
...
public class ReqTicket implements Serializable {
    ...
    private String sceneName;//使用场景名称
    private String sceneValue;//使用场景名称
    ...
    public ReqTicket(ActionName actionName, Serializable scene) {
        ...
        if (actionName == ActionName.QR_SCENE || actionName == ActionName.QR_LIMIT_SCENE) {
            ...
            if (scene instanceof Integer) {
                ...
                this.sceneId = Integer.parseInt(scene.toString());
            } else {
                throw new RuntimeException(actionName + "只支持Integer类型的场景值");
            }
        ...
        this.setSceneValue(scene.toString());
    }
